...
/*   Updated: 2022/02/23 18:29:22 by lraffin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	error_number_ceiling_floor(t_map *map_info, t_data *data)
{
	if (map_info->ceiling_floor[0] != 1 || map_info->ceiling_floor[1] != 1)
		ft_exit_parsing(data, CEILING_FLOOR_WRONG_NUMBER_LINES);
	return (FAILURE);
}
	...
	char *ceiling_or_floor, int *check)
{
	...
	if (data->map_info->file_cub[i][1] != ' '
		...
		ft_exit_parsing(data, CEILING_FLOOR_WRONG_2ND_LETTERS);
	(*check)++;
	if ((*check) == 2)
		ft_exit_parsing(data, CEILING_FLOOR_CALLED_TOO_MANY);
