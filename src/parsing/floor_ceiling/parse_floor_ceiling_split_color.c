...
/*   Updated: 2022/02/03 18:05:02 by vbachele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	check_rgb_errors(char *tmp_split, t_data *data, char **free)
{
	int	color;
...
static int	check_rgb(t_color color, t_data *data, int c)
{
	...
	while (tmp_split[i])
	{
		check_rgb_errors(tmp_split[i], data, tmp_split);
...
int	check_colors_and_add_to_struct(t_data *data, char **ceiling_floor_data)
{
	...
	{
		check_rgb(data->sprites->ceiling_color, data, 1);
		c->hex_color = ft_rgb_to_color(0, c->r, c->g, c->b);
	}
	else if (ceiling_floor_data[0][0] == 'F')
	{
		check_rgb(data->sprites->floor_color, data, 2);
