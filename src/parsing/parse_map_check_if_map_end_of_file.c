...
// je check si la premiere ligne de la map existe bien et que tous les autres criteres sont bien remplis
...
int	map_if_in_good_place(t_map *map_info)
{
	...
	while (map_info->file_cub[++i])
	{
		if (!check_walls_colors(map_info->file_cub[i], &count))
			...
			continue;
		else if (!first_or_last_line_of_map_exist(map_info->file_cub[i]) && count == 6)
		{
